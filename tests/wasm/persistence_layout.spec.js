...
async function saveLayoutActionCenter(page, screen, consoleMessages) {
  ...
  // Both clicks are real page input. Keeping serialization and saveFileContent
  // inside the QAction activation preserves the browser user gesture.
  ...
  await expect.poll(
    () => consoleMessages.filter(message => message.includes('PJ_WASM_MENU_ACTION name=save_layout')).length,
    { timeout: 10000 },
